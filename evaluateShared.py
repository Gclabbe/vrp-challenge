    ...
    def toProblemString(self):
        ...
        for idx, load in enumerate(self.loads):
            s += (
                str(idx + 1)
                + " "
                + load.pickup.toString()
                + " "
                + load.dropoff.toString()
                + "\n"
            )
        return s
...
def loadSolutionFromString(solutionStr):
    ...
    while True:
        ...
        if ("[" not in line) or ("]" not in line):
            return (
                schedules,
                "Solution format incorrect. Expected all lines to be in format [{load_id}, {load_id}, ...], but got this: "
                + line,
            )
        line = line.strip()
        line = line.replace("[", "")
...
def printSolutionFormatNag():
    print(
        "Program should only print a solution (no debugging messages) in format that looks like this:"
    )
    print("[1,4,9,7]")
    print("[5,2,3,8]")
    print("[10,6]")
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--cmd", help="Command to run your program (not including a problem file)"
    )
    ...
    for inputFile in files:
        ...
        if runTime > 30:
            print(
                "\t\tRun time constraint of 30s exceeded! Please reduce program runtime!"
            )
        sumRunTime += runTime
